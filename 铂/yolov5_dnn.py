...
class yolov5():
    def __init__(self, onnx_path, confThreshold=0.25, nmsThreshold=0.45):
        ...
        self.confThreshold = confThreshold # nms置信度
        self.nmsThreshold = nmsThreshold # nms-iou
    ...
    def numpy_nms(self, boxes :array, scores :array, iou_threshold :float):
        ...
        while idxs.size > 0:  # 统计数组中元素的个数
            max_score_index = idxs[-1] # 拿到最大分数框的索引
            max_score_box = boxes[max_score_index][None, :]
            keep.append(max_score_index) # 把最大分数框放到keep
...
def mult_test(onnx_path, img_dir, save_root_path, video=False):
    ...
    if video:
        ...
        while cap.isOpened():
            ok, frame = cap.read()
            cv2.putText(frame, 'press q or esc to exit', (27, 27), cv2.FONT_HERSHEY_SIMPLEX, 1, (55, 255, 155), 2)
            if not ok:
                break
